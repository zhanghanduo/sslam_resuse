...
void new_sequence()
{
    ...
    gpsgraph.gpsgraph_visualization->reset();
    gpsgraph.publish();
}
...
void gps_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& gps_msg) {
    ...
    gpsgraph.inputGPS_xyz(t, x_, y_, z_, pos_accuracy_x, pos_accuracy_y);
//    cout << "x: " << pos_accuracy_x << " y: " << pos_accuracy_y << endl;
}
...
//        gpsgraph.inputOdom(t, T_, R_q);
    ...
    for (auto point : point_msg_->points)
    {
        ...
        p_3d.y = point.y;
        p_3d.z = point.z;
        Eigen::Vector3d tmp = gpsgraph.r_drift * Eigen::Vector3d(p_3d.x, p_3d.y, p_3d.z) + gpsgraph.t_drift;
...
//    ROS_INFO("vio_callback!");
    ...
    vio_t = gpsgraph.w_r_vio * vio_t + gpsgraph.w_t_vio;
    vio_q = gpsgraph.w_r_vio * vio_q;

    vio_t = gpsgraph.r_drift * vio_t + gpsgraph.t_drift;
    vio_q = gpsgraph.r_drift * vio_q;
...
{
    ros::init(argc, argv, "gps_graph_node");
    ros::NodeHandle n("~");
    ros::NodeHandle nh_pub;
    gpsgraph.registerPub(nh_pub);
    ...
    gpsgraph.loadVocabulary(vocabulary_file);
...
//	m_camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(cam0Path);
    ...
    if(GPS_INIT)
    {
        ...
        if(sharedGPS_info != nullptr) {
            ...
            gpsgraph.gps_0_q = Quaterniond(gps_info.pose.pose.orientation.w, gps_info.pose.pose.orientation.x,
                                            gps_info.pose.pose.orientation.y, gps_info.pose.pose.orientation.z);

            gpsgraph.gps_0_trans = Vector3d(gps_info.pose.pose.position.x,
                                             gps_info.pose.pose.position.y, gps_info.pose.pose.position.z);

            gpsgraph.load_gps_info = true;
...
//    Subscriber<geometry_msgs::PoseWithCovarianceStamped> pos_msg_;
    ...
    pos_msg_.subscribe(n, keyframe_pose_topic, 50);

    // Exact time image topic synchronizer
    typedef sync_policies::ApproximateTime
            <sensor_msgs::Image, sensor_msgs::PointCloud, nav_msgs::Odometry> ApproxPolicy;
    typedef Synchronizer<ApproxPolicy> ApproxSync;
    boost::shared_ptr<ApproxSync> approx_sync_;

    typedef sync_policies::ApproximateTime
            <sensor_msgs::Image, sensor_msgs::PointCloud, nav_msgs::Odometry,
            obstacle_msgs::MapInfo> ApproxPolicy_dy;
...
//        sub_dynamic = n.subscribe(CUBICLE_TOPIC, 10, dymask_callback);
        ...
        approx_sync_->registerCallback(boost::bind(&multi_callback, _1, _2, _3));
    }
