...
class PoseKalmanFilter
{
  ...
  */
  ...
  void init(const double t0, const Eigen::Vector3d& z0, const Eigen::Matrix3d& r0);

  /**
   * Update the estimated state with measurement value.
   */
  void update(const Eigen::VectorXd& z);
  void update(const double t, const Eigen::Vector3d& z, const Eigen::Matrix3d& r);
  void update(const double dt, const Eigen::VectorXd& z, const Eigen::MatrixXd& A);

  /**
   * Create a new System dynamic matrix from new dt
   */
  static Eigen::MatrixXd createSystemDynamics(double dt);
  ...
  void getPoseState(Eigen::Vector3d& z, Eigen::Matrix3d& r);
  double getTime();
