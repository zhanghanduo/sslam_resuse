                                                   ...
                                                   const std::vector<SDMatch>& stereo_matches)
{
  ...
  /* Relate key points seen from both reference cameras */
  bearingVectors_t bearingVectors1;
  bearingVectors_t bearingVectors2;

  /* Defining corresponding bearing vectors for reference points triangulation using cameras 1 and 2 */
  for(const auto & stereo_match : stereo_matches){

    Point2d kp1 = kps1[stereo_match.m1vs2.queryIdx].pt;
    Point2d kp2 = kps2[stereo_match.m1vs2.trainIdx].pt;
   ...
   * but StereoCamera framework (and openGV) supports viewpoints not centered on the left cameras. Thats why we've got to
